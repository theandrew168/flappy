...
// Building:
// gcc font.c -std=c99 -lGL -lglfw -lGLU -lm
...
static int
stb_easy_font_draw_segs(float x, float y, unsigned char *segs, int num_segs, int vertical, stb_easy_font_color c, char *vbuf, int vbuf_size, int offset)
{
    int i,j;
...
static int
stb_easy_font_height(char *text)
{
    float y = 0;
    int nonempty_line=0;
    while (*text) {
...
void
print_string(float x, float y, char *text, float r, float g, float b)
{
    char buffer[99999] = { 0 }; // ~500 chars

    int quads = stb_easy_font_print(x, y, text, NULL, buffer, sizeof(buffer));

    glColor3f(r,g,b);
    glEnableClientState(GL_VERTEX_ARRAY);

    // 2 floats (8 bytes) per vertex,
